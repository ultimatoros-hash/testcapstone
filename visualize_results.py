...
def load_data_and_model():
    ...
    # DIAGNOSTIC: Vérification des dossiers
    available_classes = [d for d in os.listdir(DATA_DIR) if os.path.isdir(os.path.join(DATA_DIR, d))]
    print(f"📂 Dossiers détectés dans {DATA_DIR}: {available_classes}")
    ...
    val_ds = tf.keras.utils.image_dataset_from_directory(
        ...
        subset="validation", 
        seed=123,
        shuffle=True, # Change ici si c'était à False
        image_size=IMG_SIZE, 
...
def run_evaluation(model, dataset, class_names):
    ...
    # Sécurité : On s'assure que y_true contient des donnée
    if len(y_true) == 0:
        print("❌ Erreur: Aucune donnée de validation trouvée.")
        return
    ...
    print(f"📊 Classes présentes dans le split de validation : {np.unique(y_true)}")

    with open(os.path.join(PLOT_DIR, "metrics_report.txt"), "w") as f:
        f.write(f"GLOBAL_ACCURACY: {acc:.5f}\n") 
        f.write("\n" + report)

    cm = confusion_matrix(y_true, y_pred, labels=all_indices, normalize='true')
...
def plot_tsne(model, dataset, class_names):
    ...
    sns.scatterplot(x=result[:,0], y=result[:,1], hue=label_names, palette=PALETTE, s=70, alpha=0.8, edgecolor="k")
